...
const check = async (n) => {
  ...
  await p.waitForTimeout(120); // settle transition, but < 3600 tick
  const r = await p.evaluate((n)=>{
    const btn=document.querySelector(`[data-testid="button-why-${n}"]`);
    // newest image = last matching node in AnimatePresence
    const imgs=[...document.querySelectorAll('[data-testid^="why-rcc-image-"]')];
    const img=imgs[imgs.length-1];
    const isGold = getComputedStyle(btn).backgroundColor; // active -> primary gold
    return { btnActiveGold: isGold, imgTestid: img?.getAttribute('data-testid'), src:(img?.getAttribute('src')||'').split('/').pop(), imgCount: imgs.length };
  }, n);
  console.log(`click button-why-${n} ->`, JSON.stringify(r));
};
await check(3); await p.mouse.move(5,5); await p.waitForTimeout(150);
await check(0); await p.mouse.move(5,5); await p.waitForTimeout(150);
await check(2);
await b.close();
